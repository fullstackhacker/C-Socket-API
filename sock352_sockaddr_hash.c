...
*/
int addSocket(sockaddr_sock352_t *socket){
  ...
  sockaddr_sock352_hash_t *socket_hash; //new hash object
  ...
  socket_hash= (sockaddr_sock352_hash_t *)calloc(1, sizeof(sockaddr_sock352_hash_t)); 
  socket_hash->fd = socket_fd;//set the key
  socket_hash->socket = socket; //set the socket
	
  HASH_ADD_INT(sockets, fd, socket_hash); //hash the structure into the table
...
returns the sockaddr_sock352_t if good
*/
sockaddr_sock352_t * findSocket(int socket_fd){
  sockaddr_sock352_hash_t *socket_hash; //structure pointer
...
int deleteSocket(int socket_fd){
  sockaddr_sock352_hash_t *socket_hash; //strcuture pointer
